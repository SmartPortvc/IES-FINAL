...
import { sendHodInvitation, validateEmail } from '../utils/emailService';
import Card from './ui/Card';
import Button from './ui/Button';
import FormField from './ui/FormField';

const InviteHodForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const hodId = await createHodInvitation(currentEmail);
      ...
      try {
        await sendHodInvitation(hodId, currentEmail);
        ...
        setError(`Email delivery failed. Please use the registration link below to share with the HOD.`);
      }
      ...
      console.error('Error inviting HOD:', err);
  ...
  return (
    <Card title="Invite Head of Department (HOD)" icon={<Mail className="h-6 w-6 text-seagreen-600" />}>
      {success && (
        <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-4">
          <div className="flex items-start">
            <CheckCircle className="h-5 w-5 text-green-600 mr-2 mt-0.5" />
            <div>
              <p className="font-medium">HOD invitation created successfully!</p>
              <p className="mt-1 text-sm">
                {emailSent 
                  ? `An email with registration instructions has been sent to ${invitedEmail}.`
                  : "Please share the registration link with the HOD."}
              </p>
              ...
              {registrationLink && (
                <div className="mt-4 p-3 bg-white rounded border border-gray-200">
                  ...
                  <p className="mt-2 text-xs text-gray-500">
                    This link will be valid for 7 days. The HOD can use it to complete registration.
                  </p>
                </div>
              )}
            </div>
