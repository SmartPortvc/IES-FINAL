...
const PortRegistration: React.FC = () => {
  const { portId } = useParams<{ portId: string }>();
  const navigate = useNavigate();
  const { registerPortUser } = useAuth();
  ...
  const [landAllotted, setLandAllotted] = useState('');
  const [landUtilised, setLandUtilised] = useState('');
  const [unitizedBerths, setUnitizedBerths] = useState('');
  const [password, setPassword] = useState('');
  const [confirmPassword, setConfirmPassword] = useState('');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [portExists, setPortExists] = useState(false);
    ...
    const fetchPortData = async () => {
      ...
      try {
        ...
        if (portDoc.exists()) {
          ...
          if (portData.status === 'registered') {
            setError('This port has already been registered.');
            setInvitationValid(false);
            return;
          }
          
          // Check if invitation is expired (7 days)
          if (portData.invitationSentAt) {
            const invitationDate = new Date(portData.invitationSentAt.toDate());
            const now = new Date();
          ...
          setEmail(portData.email || '');
          setPortExists(true);
        } else {
          setError('Invalid registration link. Port not found.');
          setInvitationValid(false);
        }
      } catch (err) {
        console.error('Error fetching port data:', err);
        setError('Failed to load port information. Please try again later.');
        setInvitationValid(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!portId || !portExists || !invitationValid) {
    ...
    try {
      ...
      // Create user account
      await registerPortUser(email, password, portId);
      ...
      console.error('Error registering port:', err);
    ...
    <div className="min-h-screen bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-xl font-medium text-seagreen-600">Port Registration</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-8">
          <form onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label htmlFor="portName" className="block text-gray-700 font-medium mb-2">
                  Port Name
                </label>
                <input
                  type="text"
              ...
              <div>
                <label htmlFor="registeredAddress" className="block text-gray-700 font-medium mb-2">
                  Registered Address
                </label>
                <textarea
              ...
              <div>
                <label htmlFor="landAllotted" className="block text-gray-700 font-medium mb-2">
                ...
                <input
                  ...
                  id="landAllotted"
                  value={landAllotted}
                  onChange={(e) => setLandAllotted(e.target.value)}
                  className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-seagreen-500"
                  required
                />
              </div>
              ...
              <div>
                <label htmlFor="landUtilised" className="block text-gray-700 font-medium mb-2">
                  Land utilised (acres)
                </label>
                <input
                  type="text"
                  id="landUtilised"
                  value={landUtilised}
                  onChange={(e) => setLandUtilised(e.target.value)}
                  className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-seagreen-500"
                  required
                />
              </div>
              ...
              <div>
                <label htmlFor="unitizedBerths" className="block text-gray-700 font-medium mb-2">
                  No of berths
                </label>
                <input
                  type="text"
                  id="unitizedBerths"
                  value={unitizedBerths}
                  onChange={(e) => setUnitizedBerths(e.target.value)}
                  className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-seagreen-500"
                  required
                />
              </div>
...
export default PortRegistration;